...
uint16_t buffer_index = 0;               // It holds the index of the buffer of TELIT.
...
void send_message_to_telit() {
    ...
    #ifdef DEBUG
        printf("[DBG] The data in usb_buffer as HEX: ");
        int index = 0;
        while (usb_buffer[index] != '\0') {
                printf("%x ", usb_buffer[index]);
                index++;
        }
        printf("\n");
        printf("[DBG] The data in usb_buffer as CHAR: %s\n", usb_buffer);
    #endif
    ...
    #ifdef DEBUG
        printf("[DBG] The data with CRLF: %X\n", concat_buffer);
    #endif
    ...
    #ifdef DEBUG
        puts("[DBG] Message sent into the TELIT module.");
    #endif
}
...
int main(){
    ...
    */
    ...
    #ifdef DEBUG
        printf("[DBG] The device has been started.\r\n");
    #endif
